...
def _site() -> str:
    ...
    return "https://example.invalid/Colorado_Construction_Intelligence/"
...
def _fail(collector, exc: Exception, prev: dict | None, existing: dict[str,Permit], generated: str, today: date) -> dict:
    ...
    age=(today-date.fromisoformat(newest)).days if newest else None; freshness="fresh" if age is not None and age<=threshold else ("stale" if age is not None else "unknown")
    last=(prev or {}).get("last_success_at")
    for p in cached: classify_permit(p)
    note=f"{type(exc).__name__}: {exc}"; note=("Live collection failed; cached permits retained. "+note) if cached else note
...
def run(root: Path) -> dict:
    now=datetime.now(timezone.utc).replace(microsecond=0); generated=now.isoformat(); store=root/"data"/"permits.json"; public=root/"public"
